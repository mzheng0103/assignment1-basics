...
def _pretokenize(args):
    ...
    removed_special_tokens = re.split("|".join([re.escape(t) for t in special_tokens]), chunk)
    compiled_pat = re.compile(PAT)
    pretoken_dict = collections.Counter()

    for small_chunk in removed_special_tokens:
        pretoken_dict.update(
            tuple(bytes([b]) for b in pt.group(0).encode("utf-8"))
            for pt in re.finditer(compiled_pat, small_chunk)
        )
...
def run_train_bpe(
    ...
    **kwargs,
    ...
    for st in special_tokens:
        vocab[current_vocab_size] = st.encode("utf-8")
    ...
    with open(input_path, "rb") as f:
        ...
        for start, end in zip(boundaries[:-1], boundaries[1:]):
            ...
            chunks.append((chunk, special_tokens))  # pack args

        with Pool(num_processes) as pool:
            results = pool.map(_pretokenize, chunks)
